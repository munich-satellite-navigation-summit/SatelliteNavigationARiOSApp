...
public class LinePointer : MonoBehaviourWrapper
{
    ...
    [SerializeField]
    [HideInInspector]
    private static int _NavAreaMask = ~0; // Initialize to all
    ...
    private static bool CalculateParabolicCurve(Vector3 p0, Vector3 v0, Vector3 a, float dist, int points, List<Vector3> outPts, out Vector3 normal, Vector3 dest)
    {
        ...
        for (int i = 0; i < points; i++)
        {
            ...
            Vector3 norm;
            bool endOnNavmesh;
            bool cast = Linecast(last, next, out endOnNavmesh, out castHit, out norm);
            if (cast)
            {
                outPts.Add(castHit);
                normal = norm;
    ...
    public static bool Linecast(Vector3 p1, Vector3 p2, out bool pointOnNavmesh, out Vector3 hitPoint, out Vector3 normal)
    {
        RaycastHit hit;
        Vector3 dir = p2 - p1;
    ...
    {
        Vector3 velocity = transform.TransformDirection(_destination.position);
        Vector3 d;
    ...
    private float ClampInitialVelocity(ref Vector3 velocity, out Vector3 velocity_normalized)
    {
        ...
        if (angle > 45)
        {
            ...
            velocity *= _destination.position.magnitude;
            angle = 45;
        }
        else
